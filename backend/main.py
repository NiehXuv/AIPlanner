...
# Helper function to convert interests dictionary to a vector
def interests_to_vector(interests_dict: Dict[str, float], all_interests: list) -> np.ndarray:
...
# Helper function to generate a plan using Gemini
async def generate_plan_with_gemini(location: str, start_date: str, days: int, interests: Dict[str, float], budget: float, pois: list, delay=10, max_output_tokens=1000):
    try:
        model = genai.GenerativeModel("gemini-1.5-flash")
        prompt = (
            f"Create a {days}-day travel plan for {location} starting on {start_date}. "
            f"The traveler's interests are: {', '.join([f'{k} ({v})' for k, v in interests.items()])}. "
            f"Their budget is {budget} USD. Use the following points of interest: "
            f"{', '.join([poi['name'] for poi in pois])}. "
            "Return the result as a JSON object with a 'plan' field containing the day-by-day itinerary "
            "and a 'Hotel' field suggesting a hotel."
        )
        ...
        logging.info(f"Gemini response (raw): {result}")
        
        cleaned_result = clean_gemini_response(result)
        logging.info(f"Gemini response (cleaned): {cleaned_result}")
        
        if not cleaned_result or not (cleaned_result.strip().startswith("{") and cleaned_result.strip().endswith("}")):
            logging.warning(f"Incomplete Gemini response: {cleaned_result}")
        ...
        for day in range(days):
            ...
            plan[f"Day {day+1} ({date})"] = [f"Visit {pois[day % len(pois)]['name']} in {location}."]
        return {"plan": plan, "Hotel": f"{location} Hotel"}
...
async def generate_plan(request: PlanRequest):
    try:
        ...
        user_vector = interests_to_vector(request.interests, interests)
        ...
        for poi in pois:
            ...
            poi_vector = interests_to_vector(poi_interests, interests)
...
async def edit_plan(plan_id: str, request: PlanRequest):
    return {"message": "Plan editing not implemented"}

@app.get("/hotels/search")
async def search_hotels(location: str, checkin: str, checkout: str):
    return {"message": "Hotel search not implemented"}
